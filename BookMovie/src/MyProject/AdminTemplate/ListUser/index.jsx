import { useSelector, useDispatch } from "react-redux";
import { Management } from "./slice";
import { useEffect } from "react";
import { useState } from "react";
import { SearchUser } from "./Search";
import User from "../User";
export default function ListUser() {
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(Management());
  }, []);
  const {
    loading,
    data = [],
    error,
  } = useSelector((state) => state.ManaReducer);
  const dataSearch = useSelector((state) => state.SearchUserReducer.data);
  const loadingSearch = useSelector((state) => state.SearchUserReducer.loading);
  const errorSearch = useSelector((state) => state.SearchUserReducer.error);
  const [keyword, setKeyword] = useState({
    tuKhoa: "",
  });

  const handleOnchange = (e) => {
    const { name, value } = e.target;
    setKeyword({
      [name]: value,
    });
    if (value.trim() === "") {
      dispatch(Management());
    }
  };
  const handleSubmit = (e) => {
    e.preventDefault();

    dispatch(SearchUser(keyword.tuKhoa));
  };

  const renderUser = () => {
    if (loading || loadingSearch) {
      return (
        <tr>
          <td colSpan={5} className="text-center py-4">
            <div className="inline-flex justify-center items-center">
              <svg
                className="w-8 h-8 text-gray-200 animate-spin fill-blue-600"
                viewBox="0 0 100 101"
                fill="none"
              >
                <path
                  d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                  fill="currentColor"
                />
                <path
                  d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                  fill="currentFill"
                />
              </svg>
              <span className="sr-only">Loading...</span>
            </div>
          </td>
        </tr>
      );
    }
    if (error || errorSearch) {
      return (
        <tr>
          <td colSpan={6} className="text-center text-red-600 py-4">
            Đã xảy ra lỗi khi tải dữ liệu.
          </td>
        </tr>
      );
    }

    if (dataSearch && dataSearch.length > 0) {
      return dataSearch.map((user) => <User key={user.taiKhoan} data={user} />);
    }
    if (dataSearch && dataSearch.length === 0 && keyword.tuKhoa !== "") {
      return (
        <tr>
          <td colSpan={6} className="text-center text-gray-500 py-4">
            Không tìm thấy người dùng nào phù hợp.
          </td>
        </tr>
      );
    }
    if (!data || data.length === 0) {
      return (
        <tr>
          <td colSpan={5} className="text-center py-4">
            <div className="inline-flex justify-center items-center">
              <svg
                className="w-8 h-8 text-gray-200 animate-spin fill-blue-600"
                viewBox="0 0 100 101"
                fill="none"
              >
                <path
                  d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                  fill="currentColor"
                />
                <path
                  d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                  fill="currentFill"
                />
              </svg>
              <span className="sr-only">Loading...</span>
            </div>
          </td>
        </tr>
      );
    }

    return data.map((user) => {
      return <User key={user.taiKhoan} data={user} />;
    });
  };
  return (
    <div className="container mx-auto my-10">
      <h1 className="text-center font-bold mb-5 text-2xl text-amber-600">
        Danh sách người dùng
      </h1>
      <div className="relative my-10">
        <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
          <svg
            className="w-4 h-4 text-gray-500 dark:text-gray-400"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 20 20"
          >
            <path
              stroke="currentColor"
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"
            />
          </svg>
        </div>
        <input
          type="search"
          id="search"
          onChange={handleOnchange}
          name="tuKhoa"
          className="block w-full p-4 ps-10 text-sm text-black border-1 border-black-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 "
          placeholder="Search"
          required
        />
        <button
          onClick={handleSubmit}
          type="submit"
          className="text-white absolute end-2.5 bottom-2.5 bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-red-700 dark:hover:bg-red-800 dark:focus:ring-red-900"
        >
          Tìm kiếm
        </button>
      </div>
      <div className="relative overflow-x-auto shadow-md sm:rounded-xl overflow-y-auto max-h-[600px]">
        <table className="w-full text-sm text-left text-gray-700 border border-gray-200 border-collapse">
          <thead className="sticky top-0 z-10 text-xs uppercase bg-gradient-to-r from-red-800 to-black text-white">
            <tr>
              <th scope="col" className="px-6 py-3">
                Tài Khoản
              </th>
              <th scope="col" className="px-6 py-3">
                Họ Tên
              </th>
              <th scope="col" className="px-6 py-3">
                Email
              </th>
              <th scope="col" className="px-6 py-3">
                Số điện thoại
              </th>
              <th scope="col" className="px-6 py-3">
                Mã loại người dùng
              </th>
              <th scope="col" className="px-6 py-3">
                Hành Động
              </th>
            </tr>
          </thead>
          <tbody>{renderUser()}</tbody>
        </table>
      </div>
    </div>
  );
}
